...
export function AboutUs() {
  ...
  return (
    <div>
      ...
      {/* Company Toggle — sticky, not animated per plan */}
      ...
      {/* Company Content — key forces remount on toggle */}
        ...
        <div className="max-w-[80rem] mx-auto px-6 grid md:grid-cols-2 gap-14 items-start">
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              <div className={`w-10 h-10 rounded flex items-center justify-center ${active === "icp" ? "bg-brand-primary" : "bg-brand-accent"}`}>
                {active === "icp" ? <HardHat className="w-6 h-6 text-white" /> : <Cuboid className="w-6 h-6 text-white" />}
              </div>
              <div>
                <div className={active === "icp" ? "text-brand-primary" : "text-brand-accent-strong"} style={{ fontWeight: 800 }}>
                  {co.logoText}
                </div>
            ...
            <p className={`text-sm mb-3 tracking-widest uppercase ${active === "icp" ? "text-brand-secondary" : "text-brand-secondary"}`}>
              {co.tagline}
            </p>
            <h2 className="text-brand-primary mb-5" style={{ fontSize: "1.8rem", fontWeight: 800 }}>Company Introduction</h2>
            ...
            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {co.highlights.map((h) => (
                <li key={h} className="flex items-start gap-2 text-gray-600 text-sm">
                  <CheckCircle2 className={`w-4 h-4 shrink-0 mt-0.5 ${active === "icp" ? "text-brand-secondary" : "text-brand-accent"}`} />
                  {h}
                </li>
              ))}
            </ul>
        ...
        <div className="max-w-[80rem] mx-auto px-6">
          ...
          >
            <motion.div
              ...
              variants={shouldReduceMotion ? undefined : staggerChildVariants}
            >
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 ${active === "icp" ? "bg-brand-soft" : "bg-brand-highlight"}`}>
                <Target className={`w-6 h-6 ${active === "icp" ? "text-brand-primary" : "text-brand-secondary"}`} />
              </div>
              <h3 className="text-brand-primary mb-4" style={{ fontSize: "1.3rem", fontWeight: 800 }}>Our Mission</h3>
            ...
            >
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 ${active === "icp" ? "bg-brand-soft" : "bg-brand-highlight"}`}>
                <Eye className={`w-6 h-6 ${active === "icp" ? "text-brand-primary" : "text-brand-secondary"}`} />
              </div>
              <h3 className="text-brand-primary mb-4" style={{ fontSize: "1.3rem", fontWeight: 800 }}>Our Vision</h3>
          ...
          >
            {co.values.map((v) => (
              ...
              >
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${active === "icp" ? "bg-brand-soft" : "bg-brand-highlight"}`}>
                  <v.icon className={`w-5 h-5 ${active === "icp" ? "text-primary" : "text-brand-secondary"}`} />
                </div>
                <div>
      ...
      <section className="py-20 bg-brand-primary">
        <div className="max-w-[80rem] mx-auto px-6">
          ...
          >
            ...
            <h2 className="text-white mt-2" style={{ fontSize: "2.2rem", fontWeight: 800 }}>Development Timeline</h2>
                ...
                <div key={item.year} className="w-full">
                  ...
                  >
                    <div className="flex flex-col items-center mr-4 shrink-0">
                      ...
                      <div className="w-12 h-12 rounded-full flex items-center justify-center border-2 text-xs z-10 shrink-0 bg-brand-accent border-brand-accent text-white" style={{ fontWeight: 800, fontSize: "0.65rem" }}>
                        {item.year}
                      </div>
                  ...
                  {/* Desktop layout — alternating slide directions */}
                  <motion.div
                    className={`hidden md:flex md:flex-row items-center gap-6 ${i % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse"}`}
                    variants={shouldReduceMotion ? undefined : (i % 2 === 0 ? slideFromLeftVariants : slideFromRightVariants)}
                    initial="hidden"
                    whileInView="visible"
                    viewport={VIEWPORT}
                  >
                    ...
                    <div className="md:w-16 flex justify-center shrink-0">
                      <div className="w-14 h-14 rounded-full flex items-center justify-center border-2 text-xs z-10 shrink-0 bg-brand-accent border-brand-accent text-white" style={{ fontWeight: 800, fontSize: "0.7rem" }}>
                        {item.year}
                      </div>
                    </div>
